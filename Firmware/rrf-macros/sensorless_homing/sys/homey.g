...
if !var.SensorLessHoming
  ; First pass
  G1 Y600 F2400 H1                                                             ; Move quickly to Y axis endstop and stop there (first pass)
...
if var.SensorLessHoming
  ...
  M915 Y S8 R0 F0 H200                                                         ; Set stall detection sensitivity(-128 to +127), action type, filter mode and  minimum motor full steps per second

  ; First pass
  G1 Y600 F7000 H1                                                             ; Move quickly to Y axis max and stop there

  ; Go back a few mm
  G1 Y-5 F9000                                                                 ; Go back a few mm

  ; Second pass
  G1 Y600 F3600 H1                                                             ; Move slowly to Y axis max once more
  M400                                                                         ; Wait for current moves to finish
